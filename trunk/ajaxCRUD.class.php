...
class ajaxCRUD{
	...
	################################################
    #
    # The following are utf8 encoding, files uploading's options, language's options
    ...
    function ajaxCRUD($item, $db_table, $db_table_pk, $ajaxcrud_root = "") {
        ...
        $this->loading_image_html = "<center><br /><br  /><img src=\'css/loading.gif\'><br /><br /></center>"; //changed via setLoadingImageHTML()
	...
	function showTable(){
        ...
        if ($row_count > 0){
            ...
            $table_html .= "<table align='center' name='the_table' id='the_table' cellspacing='" . $this->cellspacing . "' border=" . $this->border . ">\n";
        ...
        if ($this->add){
            $add_html .= "<center>\n";
            $add_html .= "   <input type=\"button\" value=\"".$this->messagearray["add"]." $item\" class=\"btn editingSize\" onClick=\"$('#add_form_$this->db_table').slideDown('slow');\">\n";
            ...
            $add_html .= "  <br /><h3>".$this->messagearray["newrow"]."<b>$item</b></h3>\n";
                                ...
                                if ($this->fieldInArray($field, $this->file_uploads)){
                                    ...
                                    if ($this->fieldIsEnum($this->getFieldDataType($field))){
										...
										if ((strtolower($this->getFieldDataType($field))=='text') or $this->textarea_height[$field] != ''){
											...
											{
	                                            $add_html .= "<th width='20%'>$display_field</th><td><textarea onKeyPress=\"$field_onKeyPress\" class=\"editingSize\" name=\"$field\" style='width: 97%; height: 80px;'>$field_value</textarea></td></tr>\n";
											}
                                        }
                                        else{
                                            //any ol' data will do
            ...
            if ($this->ajax_add){
                ...
                $add_html .= "<input class=\"editingSize\" type=\"submit\" value=\"".$this->messagearray["add"]." $item\">";
            }

            $add_html .= "</td><td><input style='float: right;' class=\"editingSize\" type=\"button\" onClick=\"$('#add_form_$this->db_table').slideUp('slow');\" value=\"".$this->messagearray["cancel"]."\"></td></tr>\n</table>\n";
